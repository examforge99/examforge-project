...
export async function GET(request: Request) {
  try {
    ...
    // ── Build final response — clean options array, no null options ───────────

    const result = shuffled.slice(0, limit).map((q, index) => ({
      id: q.id,
      question_number: index + 1,
      question_text: q.question_text,
      options: [q.option_1, q.option_2, q.option_3, q.option_4, q.option_5].filter(Boolean) as string[],
      subject: q.subject,
      topic: q.topic ?? null,
