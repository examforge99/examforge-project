...
export async function GET(request: Request) {
  try {
    ...
    if (userRes.error || !userRes.data) {
      await logError('STUDENT_CONTEXT_USER_NOT_FOUND', userRes.error?.message ?? 'User not found', null, user_id, null)
    ...
    const recentMetricSubjects = new Set(
      metrics.filter(m => m.last_updated > sevenDaysAgo).map(m => m.subject)
    )
    ...
    const totalQuestionsAnswered = Object.values(subjectTotals).reduce((sum, s) => sum + s.attempted, 0)
    ...
    if (recentAttempts.length >= 20) {
      const last10Acc = recentAttempts.slice(0, 10).filter(a => a.is_correct).length / 10
      const prev10Acc = recentAttempts.slice(10, 20).filter(a => a.is_correct).length / 10
    ...
    const sessionMap = new Map<string, { session_id: string; attempts: typeof recentAttempts; timestamp: string }>()
    for (const attempt of recentAttempts) {
      if (!attempt.session_id) continue
      if (!sessionMap.has(attempt.session_id)) {
        sessionMap.set(attempt.session_id, { session_id: attempt.session_id, attempts: [], timestamp: attempt.attempt_timestamp })
      }
    ...
    return Response.json({
      ...
      milestones: {
        ...
        first_70_percent_achieved: overallAccuracy >= 70,
        longest_streak: streak?.longest_streak ?? 0,
      },
      exam_info: examInfo,
    ...
    await logError('STUDENT_CONTEXT_UNHANDLED', err.message, err.stack ?? null, null, null)
