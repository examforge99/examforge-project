...
interface PlanSettings {
  ...
  prices: Record<PlanKey, number>         // naira — display only
  prices_kobo: Record<PlanKey, number>    // kobo — send to API
...
function PlanCard({
  ...
  const perMonth = plan.key === '1_month'
    ? price
    : Math.round(price / parseInt(plan.key))

  return (
    <div
      onClick={() => enabled && !loading && onSelect()}
      style={{
        position: 'relative',
      ...
      onMouseLeave={e => {
        if (!selected) {
          e.currentTarget.style.borderColor = plan.popular ? '#1d4ed8' : 'rgba(15,23,42,0.10)'
...
export default function SubscribePage() {
    ...
    const fetchSettings = async () => {
      try {
        const [settingsRes, waRes] = await Promise.all([
          ...
          fetch('/api/settings/public?key=support_whatsapp'),
        ])

        if (!settingsRes.ok) {
          const err = await settingsRes.json()
          setSettingsError(err.error ?? 'Could not load plans.')
          return
        }
        ...
        if (waRes.ok) {
          ...
          setWhatsappNumber(waData.value ?? null)
        }
      } catch {
        setSettingsError('Could not load subscription plans. Please refresh.')
      } finally {
        setLoading(false)
      }
    }
  ...
  const handleSubscribe = async () => {
    ...
    try {
      ...
      if (!res.ok || data.error) {
        setSettingsError(data.error ?? 'Payment initialization failed. Please try again.')
        return
      }
  ...
  const currentPriceKobo = settings?.prices_kobo[selectedPlan] ?? 0
      ...
      <div style={{ maxWidth: 680, margin: '0 auto', padding: '0 16px' }}>
        ...
        {paymentFailed && (
          <div style={{
            ...
            <div style={{ flexShrink: 0, marginTop: 1 }}><Icons.AlertTriangle /></div>
            <div>
        ...
        {settingsError && (
          <div style={{
            ...
            borderRadius: 12, padding: '14px 16px',
            marginTop: 24, color: '#dc2626', fontSize: 14,
          }}>
            {settingsError}
          </div>
        )}
        ...
        {!loading && settings && !settings.payments_enabled && (
          <div style={{
            ...
            marginTop: 24, color: '#92400e', fontSize: 14,
            textAlign: 'center',
          }}>
        ...
        <div style={{ marginTop: 28 }}>
          ...
          ) : settings && settings.payments_enabled ? (
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 12 }}>
              {PLANS.map(plan => (
                <PlanCard
                  key={plan.key}
                  ...
                  priceKobo={settings.prices_kobo[plan.key]}
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px 16px' }}>
              {FEATURES.map(feature => (
                <div key={feature} style={{
                  ...
                  <div style={{ color: '#16a34a', flexShrink: 0 }}><Icons.Check /></div>
                  {feature}
                </div>
              ))}
            </div>
        ...
        {!loading && settings?.payments_enabled && settings?.coupons_enabled && (
          <div style={{
            ...
            {couponError && (
              <div style={{ fontSize: 12, color: '#dc2626', marginTop: 8 }}>{couponError}</div>
            )}
            {couponApplied && couponDiscount > 0 && (
              <div style={{
                ...
                <Icons.Check /> ₦{couponDiscount.toLocaleString('en-NG')} discount applied
              </div>
            )}
          </div>
        )}
          ...
          <div style={{
            ...
            <div style={{ marginBottom: 16 }}>
              ...
              {couponApplied && couponDiscount > 0 && (
                <div style={{
                  display: 'flex', justifyContent: 'space-between',
                  fontSize: 13, color: '#16a34a', marginBottom: 8,
                }}>
